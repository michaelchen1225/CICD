...
SonarQube 是一套**靜態程式碼分析**平台，能掃描程式碼並找出 Bug、安全漏洞與不良寫法。整合進 CI/CD 之後，每次 push 就會自動掃一次，讓問題在進主線之前就被攔下來。本文聚焦在 SonarQube 本身（介紹、部署、設定檔、Quality Gate）；如何把它接進 GitLab CI/CD pipeline 留在 [02-gitlab-ci-integration.md](02-gitlab-ci-integration.md)，掃完之後怎麼讀結果、調 Quality Gate 留在 [03-quality-gate-and-results.md](03-quality-gate-and-results.md)。
...
> 在 `.gitlab-ci.yml` 裡實際怎麼寫 `rules:` 留到 [02-gitlab-ci-integration.md](02-gitlab-ci-integration.md)。本節只是先把「能跑 vs 結果是什麼」釐清。
...
**怎麼讓 CI 因 Quality Gate 失敗而停下來**：scanner 預設不等結果就退出，要加 `-Dsonar.qualitygate.wait=true` 讓它持續 polling，Quality Gate 一旦判定為 Fail，scanner 的 exit code 就會非零，pipeline 就會跟著失敗。完整的 GitLab CI 整合（cache、`rules:`、token 設定）請看 [02-gitlab-ci-integration.md](02-gitlab-ci-integration.md)；Quality Gate 在 CI 端的把關機制（timeout、`allow_failure` 切換時機）見 [03-quality-gate-and-results.md](03-quality-gate-and-results.md)。
