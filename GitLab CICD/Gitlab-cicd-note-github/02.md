...
- [基本寫法](#基本寫法)
  - [最簡單的腳本](#最簡單的腳本)
  - [多個 Job](#多個-job)
  - [before\_script \& after\_script](#before_script--after_script)
  ...
  - [跨 job 重用 dependency --- cache](#跨-job-重用-dependency-----cache)
- [相依關係 \& 條件處理](#相依關係--條件處理)
  - [多階段處理 -- stages](#多階段處理----stages)
  - [設定相依關係 \& 縮短跨 stage 等待時間 -- needs](#設定相依關係--縮短跨-stage-等待時間----needs)
  - [設定相依關係 \& 限定繼承 artifacts -- dependencies](#設定相依關係--限定繼承-artifacts----dependencies)
  - [若當前 Job 失敗，後續 stage 仍然執行 -- allow\_failure](#若當前-job-失敗後續-stage-仍然執行----allow_failure)
...
- [引用、繼承、傳送相關](#引用繼承傳送相關)
  ...
  - [把變數值傳給後續的 Job --- artifacts:reports:dotenv](#把變數值傳給後續的-job-----artifactsreportsdotenv)
- [常用的分支條件](#常用的分支條件)
  - [指定分支執行特定 Job --- rules + COMMIT\_REF\_NAME](#指定分支執行特定-job-----rules--commit_ref_name)
  - [指定分支執行特定 Job -- only](#指定分支執行特定-job----only)
  - [指定哪些分支不執行特定 Job -- except](#指定哪些分支不執行特定-job----except)
...
### 跨 job 重用 dependency --- cache

> 詳細操作與 runner 端的儲存設定請參考 [08-cache.md](08-cache.md)，這裡只整理 `.gitlab-ci.yml` 的語法。

每個 job 都跑在乾淨的環境（docker executor 每個 job 一個新 container），沒有 cache 的話，每次 build 都會重新下載 Maven / npm / pip dependency。`cache` 可以把指定目錄打包成 `cache.zip`，下次同 key 的 job 開始時自動還原。
...
2. `cache.key` 決定 `cache.zip` 的檔名 / 隔離單位，相同 key 的 job 共用同一份 cache。
3. **要把 build tool 的 dependency 路徑改到 `cache.paths` 所指的位置**。以 Maven 為例，預設會寫到 `~/.m2/repository`（家目錄），跟 `cache.paths` 不一致 → cache 抓不到內容、永遠是空的；要用 `MAVEN_OPTS=-Dmaven.repo.local=$CI_PROJECT_DIR/.m2/repository` 強制對齊。其他工具同理（npm 的 `npm config set cache`、pip 的 `PIP_CACHE_DIR` 等）。
...
* **Protected branch 自動隔離**：

GitLab 對 protected branch 會自動在 cache key 後面加 `-protected`，避免 release / main 等 branch 的 cache 跟一般 branch 互相污染：
...
兩者對應兩個獨立的 `cache.zip`，不需要自己處理。

* **每個 branch 完全獨立 cache**（用 `$CI_COMMIT_REF_SLUG` + `fallback_keys`）：
...
`fallback_keys` 的用途：當前 key 找不到 cache 時，按順序退而求其次去抓清單裡的 key。代價是 disk 用量會隨活躍 branch 數量線性成長，需要定期清理。

* **`cache.zip` 是覆蓋制，不是累積**：同一個 cache key 永遠只有一份 `cache.zip`，每次 job 結束會直接覆蓋舊版，沒有版本歷史。要強制重置就改 key 的後綴（例如 `...-v2`）。

* **cache vs artifacts 的差別**：

| 比較項目 | `cache` | `artifacts` |
| --- | --- | --- |
| 目的 | 加速 job（重用 dependency） | 在 pipeline 內傳遞產物 / 對外輸出 |
| 跨 pipeline | 會留存（同 key 共用） | 不會（每個 pipeline 各自一份） |
| 失敗時的處理 | 不會中斷 job | 會被當成 job 產出 |
| 典型用途 | `.m2/repository`、`node_modules` | build 產物、test report、log |
...
注意：不要和 Job level 的 rules 搞混

```
workflow.rules 決定整條 pipeline 是否被建立（先過第一關）。
job.rules 在 pipeline 被建立後，決定每個 job 個別是否加入。
```
